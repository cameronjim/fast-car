...
| JETSON | 4-pin (STEER SIG, THROTTLE SIG, HEARTBEAT, GND) | all three Jetson-originated signals plus their shared return | Pin 1 (STEER SIG) is marked on the board and the plug is keyed, because a reversed plug swaps heartbeat and steering and nothing in firmware can see that. No 5 V pin: **the Jetson powers itself.** Steering and throttle go through the shifter into GPIO 3 and 4; the heartbeat is already 3.3 V and goes straight to GPIO 5. On the Jetson side, HEARTBEAT and GND come off the 40-pin header's **physical pin 7 (signal) and pin 9 (ground)** -- see `tools/jetson_heartbeat/README.md` for the pin mapping and its verification. |
