...
// invalid regardless of bounds. A non-finite BOUND is also always invalid: without that
// check a NaN min_us or max_us makes both range comparisons false and every pulse reads as
// valid, which is fail-OPEN. `min_us`/`max_us` are not validated relative to each other here
// -- this is a pure range check; a caller that supplies min_us > max_us gets "always
