...
void PwmOutputDriver::start() {
  ...
  steering_.start(steering_period_ns_,
                  pulse_us_to_duty_ns(neutral.steering_us, steering_period_ns_));
  throttle_.start(throttle_period_ns_,
                  pulse_us_to_duty_ns(neutral.throttle_us, throttle_period_ns_));
}

PulsePair PwmOutputDriver::update(const CommandState& state) {
  const PulsePair pulses = compute_outputs(config_, state);
  steering_.write_duty_ns(pulse_us_to_duty_ns(pulses.steering_us, steering_period_ns_));
  throttle_.write_duty_ns(pulse_us_to_duty_ns(pulses.throttle_us, throttle_period_ns_));
  return pulses;
}
...
void PwmOutputDriver::force_neutral() noexcept {
  ...
  try {
    steering_.write_duty_ns(pulse_us_to_duty_ns(neutral.steering_us, steering_period_ns_));
  } catch (...) {  // NOLINT(bugprone-empty-catch)
  }
  try {
    throttle_.write_duty_ns(pulse_us_to_duty_ns(neutral.throttle_us, throttle_period_ns_));
  } catch (...) {  // NOLINT(bugprone-empty-catch)
  }
}
...
void PwmOutputDriver::stop() noexcept {
  ...
  stopped_ = true;
  const PulsePair neutral = neutral_outputs(config_);
  steering_.stop(pulse_us_to_duty_ns(neutral.steering_us, steering_period_ns_));
  throttle_.stop(pulse_us_to_duty_ns(neutral.throttle_us, throttle_period_ns_));
}
